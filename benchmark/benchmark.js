...
import fuzzaldrinPlus from '../src/fuzzaldrin-plus';
import legacy from 'fuzzaldrin';

let lines = fs.readFileSync(path.join(__dirname, 'data.txt'), 'utf8').trim().split('\n');
let forceAllMatch = {maxInners: -1};
let mitigation = {maxInners: Math.floor(0.2 * lines.length)};
...
let results = fuzzaldrinPlus.filter(lines, 'index');
...
startTime = Date.now();
let query = 'index';
let prepared = fuzzaldrinPlus.prepareQuery(query);
for (var line of lines) { fuzzaldrinPlus.match(line, query, {preparedQuery: prepared}); }
console.log(`Matching ${lines.length} results for 'index' took ${Date.now() - startTime}ms (Prepare in advance)`);

startTime = Date.now();
for (line of lines) { fuzzaldrinPlus.match(line, query); }
console.log(`Matching ${lines.length} results for 'index' took ${Date.now() - startTime}ms (cache)`);

startTime = Date.now();
for (line of lines) { legacy.match(line, query); }
